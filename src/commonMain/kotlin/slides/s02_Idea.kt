...
val sectionIdea by Slide {
    SectionSlide(
        number = "02",
        title = "The Idea",
        subtitle = "How to Compose once over the wire",
    )
}
...
val composeIsFun by PreparedSlide(
    context = SpeakerNotes(
...
val composeRemotely by Slide(
    ...
    Column(
        ...
        Row(verticalAlignment = Alignment.Bottom) {
            Text("What if we could Compose… ", fontFamily = fonts.title, color = Livewire.Cream, fontSize = 26.sp)
            Text("remotely", fontFamily = fonts.title, color = Livewire.Amber, fontSize = 26.sp, fontStyle = FontStyle.Italic)
        }
...
val remoteComposeYes by PreparedSlide(
    context = SpeakerNotes(
...
val uiVsRuntime by Slide(
    ...
    TitledSlide(title = "", kicker = "// COMPOSE BY ANY OTHER NAME") {
        Row(verticalAlignment = Alignment.Bottom) {
            ...
            Text("UI", fontFamily = fonts.title, color = Livewire.Amber, fontSize = 24.sp, fontStyle = FontStyle.Italic)
            Text("  vs.  ", fontFamily = fonts.title, color = Livewire.Gray, fontSize = 24.sp)
            Text("Compose ", fontFamily = fonts.title, color = Livewire.Cream, fontSize = 24.sp)
            Text("Runtime", fontFamily = fonts.title, color = Livewire.Red, fontSize = 24.sp, fontStyle = FontStyle.Italic)
        }
        Spacer(Modifier.height(14.dp))
        Row(Modifier.fillMaxSize()) {
            Column(Modifier.weight(1f), horizontalAlignment = Alignment.CenterHorizontally) {
                Text(
                    line { b("UI "); i("(i.e. Jetpack Compose)") },
...
val otherTalks by Slide(
    ...
    TitledSlide(title = "Check out these other talks", kicker = "// COMMUNITY") {
        Row(Modifier.fillMaxSize(), horizontalArrangement = Arrangement.spacedBy(24.dp)) {
            listOf(
                Triple(Res.drawable.talk_thumb_1, "The Compose Runtime, Demystified", "Leland Richardson"),
                Triple(Res.drawable.talk_thumb_2, "Demystifying the Compose Runtime & Compiler", "Andrew Bailey"),
