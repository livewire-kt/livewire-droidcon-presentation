...
Slide(
  context =
    SpeakerNotes(
      """
        ERIC:
        Your instinct here is probably to make the app the server. It has the UI and the data, so the desktop app should connect in to fetch it, right?

        Turns out we have to do the reverse: the desktop is the server and every app dials out to it.

        Here's why: A USB cable is not an IP network. The phone has no route to your computer. It literally can't address it.
  ...
  context =
    SpeakerNotes(
      """
        ...
        Every client sends a tiny beacon, which is designed to be tiny and self-contained.

        One packet gives the host everything it needs to draw a row in the picker: instanceId, app name, app icon, and a whole bunch of other metadata.

        For the curious: we started out with this as JSON. It's now an artisanal, hand-rolled, length-prefixed binary format with a protocol version at the very front. Why? Because base64'd app icons are WAY BIGGER than byte arrays.

        You can see in the diagram that we have two-transport split. The deciding question is: does this client share the host's loopback? Desktop and the simulator do, so they can just fire a UDP datagram at a known localhost port every couple seconds and the host listens.

        Android and a physical iOS device sit behind a USB connection, and UDP broadcast can't cross that, so those clients run a tiny TCP server and the host opens it, reads one packet, and closes.

        We use a port range instead of one port so you can run several instances of an app on a device.
      """.trimIndent()
    )
) {
