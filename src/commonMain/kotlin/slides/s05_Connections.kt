    ...
    TitledSlide(title = "The host is the server", kicker = "// CONNECTIONS") {
        Row(Modifier.fillMaxSize(), verticalAlignment = Alignment.CenterVertically) {
            Column(Modifier.weight(1.1f)) {
                ...
                Bullet(line {
                    t("So loopback is the one address both ends share — every client does the same trivial thing: dial ")
...
val desktopEasy by Slide(
    ...
    TitledSlide(title = "Desktop: the easy one", kicker = "// CONNECTIONS") {
        Bullet(line { t("It's a KMP tool and desktop is where iteration is fastest — so we started here.") })
        ...
        Row(Modifier.fillMaxSize(), verticalAlignment = Alignment.CenterVertically) {
            Column(Modifier.weight(1.2f)) {
                Bullet(line {
                    ...
                    code("usbmuxd")
                    t(", Apple's undocumented daemon that powers connections for Finder, Apple Music, Xcode, iTunes (RIP), etc")
                })
                Bullet(line {
                    code("usbmuxd listen")
...
val physicalIos3 by Slide(
    ...
    TitledSlide(title = "Physical iOS", kicker = "// CONNECTIONS") {
        Bullet(line {
            ...
            t(" only reaches ")
            em("host → device")
            t(". To keep “the app dials loopback,” both ends run a socket forwarder and we stitch a loopback tunnel across the cable ourselves.")
        })
        Spacer(Modifier.height(8.dp))
        Image(
...
val e2eEncryption2 by Slide(
    ...
    TitledSlide(title = "E2E Encryption", kicker = "// CONNECTIONS") {
        Bullet(line {
            t("The first frames are the handshake. No fixed client/server role, key ownership is decided by comparing public keys.")
        })
...
val stayingConnected by PreparedSlide(
    context = SpeakerNotes(
