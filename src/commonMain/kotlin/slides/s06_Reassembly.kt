...
val sectionReassembly by Slide(
    ...
    SectionSlide(
        number = "04",
        title = "Re-assembly",
        subtitle = "Not all parts included.",
    )
}

val renderingOnHost by PreparedSlide(
    stepCount = 1,
    context = SpeakerNotes(
...
val renderingTree by PreparedSlide(
    context = SpeakerNotes(
...
val renderingNode by PreparedSlide(
    stepCount = 3,
    context = SpeakerNotes(
        listOf(
            0..0 to "For layout and other nodes can can handle children iterate through them…",
            1..1 to "… then keying each children to the composition key hash generated from the " +
                "client so that the client stays the source of truth in this \"dual-composition\" " +
                "kind of setup",
            2..2 to "Then fold their LivewireModifier into a Compose UI modifier, and then feed " +
...
val diffing by PreparedSlide(
    ...
    slideContent {
        TitledSlide(title = "Composition over the wire", kicker = "// DIFFING") {
            Bullet(line {
                t("First frame serializes the full tree, after that the Applier records what the runtime ")
                i("did")
            })
...
val backpressure by Slide(
    context = SpeakerNotes(""),
) {
    TitledSlide(title = "Handling backpressure", kicker = "// DIFFING") {
        Bullet(line { t("Problem: composition can produce frames far faster than the wire drains them") })
        Bullet(line {
            t("A custom ")
        ...
        Bullet(line { t("Host is independent enough that throttling doesn't hurt us - latest-wins, no lag") })
    }
}

val actions by Slide(
    context = SpeakerNotes(
...
val definingIntention by PreparedSlide(
    context = SpeakerNotes(
    ...
    val left = rememberSourceCode(language = "kotlin", key = "actionsLeft") {
        // language=kotlin
        """
        @Serializable
    ...
    val right = rememberSourceCode(language = "kotlin", key = "actionsRight") {
        // language=kotlin
        """
        @Serializable
...
val declaringIntention by PreparedSlide(
    stepCount = 4,
    context = SpeakerNotes(
        listOf(
            0..0 to "Now we need an API to let developers declare this intentions",
            1..1 to "First, we capture the unique composition key for this action/node so that " +
                "we can uniquely relate the input from the user when sent back from the host",
            2..2 to "Then, in a `LaunchedEffect` we observe all actions of our type, filtered to " +
                "our unique composition id and then execute the saved lambda-action when we " +
                "observe this event.",
            3..3 to "Lastly, we remember and return this action, i.e. intention, to the caller " +
...
val clickingBackwards by PreparedSlide(
    context = SpeakerNotes(
...
val deliveringAction by PreparedSlide(
    context = SpeakerNotes(
