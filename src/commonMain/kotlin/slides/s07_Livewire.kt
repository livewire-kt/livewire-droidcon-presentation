...
val sectionLivewire by Slide(
    ...
    SectionSlide(
        number = "05",
        title = "Livewire",
        subtitle = "Something here, idk what",
    )
}

val demoScreens by Slide(
    stepCount = 5,
    context = SpeakerNotes(
        listOf(
            1..1 to "Run custom queries. View schemas. View tables. Database queries through the " +
                "same SQL driver your app uses.",
            2..2 to "Network: one line in your OkHttp/Ktor setup; requests, timings, headers, " +
                "formatted bodies, even images render. It reads the app's traffic because it IS " +
                "the app.",
            3..3 to "Recomposition plugin, which is what I'm personally most proud of.\n\n" +
            ...
            4..4 to "When you select a node, it tells you WHY it invalidated (if it can) - which " +
...
val introspection by Slide(
    context = SpeakerNotes("or it observes its own emissions, forever"),
) {
