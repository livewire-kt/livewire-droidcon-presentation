...
data class LivewireFonts(
    val title: FontFamily,
    val body: FontFamily,
    val mono: FontFamily,
)

val LocalLivewireFonts = staticCompositionLocalOf<LivewireFonts> {
    error("LivewireFonts not provided — wrap content in LivewireTheme")
}

val livewireCodeTheme: SourceCodeTheme = { cls ->
    when (cls) {
        "default", "subst", "punctuation", "operator", "tag",
        -> SpanStyle(color = Livewire.CodePlain)

        "keyword", "literal",
        -> SpanStyle(color = Livewire.CodeKeyword)

        "title", "class", "type", "name", "built_in", "function", "symbol", "variable", "property", "attr", "meta",
        -> SpanStyle(color = Livewire.CodeType)

        "comment", "quote", "doctag",
        -> SpanStyle(color = Livewire.CodeComment)

        "number",
        -> SpanStyle(color = Livewire.CodeNumber)

        "string", "regexp",
        -> SpanStyle(color = Livewire.CodeString)
...
fun LivewireTheme(content: @Composable () -> Unit) {
    val fonts = LivewireFonts(
        ...
        body = FontFamily(
            ...
            Font(Res.font.googlesans_medium_italic, weight = FontWeight.Medium, style = FontStyle.Italic),
            Font(Res.font.googlesans_semibold, weight = FontWeight.SemiBold),
            Font(Res.font.googlesans_semibold_italic, weight = FontWeight.SemiBold, style = FontStyle.Italic),
